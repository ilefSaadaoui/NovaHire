      ...
      <div class="luxury-modal-content anim-scale-up" @click.stop>
        ...
        <div class="modal-header-premium">
          <div class="modal-title-group">
            <div class="icon-box-themed">
              <UserCheck :size="22" stroke-width="2.5" />
            </div>
            <div class="modal-title-text">
              <div class="modal-badge">Edition Membre</div>
              <h3 class="celestial-modal-title">Modifier un Membre</h3>
              <p class="modal-intro">Mettez à jour les informations du membre de l'équipe</p>
            </div>
          </div>
        ...
        <form @submit.prevent="$emit('submit')" class="modal-form-wrapper">
          ...
          <div class="modal-footer-lux">
            ...
            <button type="submit" class="btn-save-member" :disabled="loading">
...
import { Mail, Loader2, User, UserCheck, X, Save, Building } from 'lucide-vue-next'
...
.btn-save-member::after {
  content: ''; position: absolute; top: 0; left: -100%;
  width: 100%; height: 100%;
  background: linear-gradient(90deg, transparent, rgba(255,255,255,0.2), transparent);
  transition: 0.5s;
}
.btn-save-member:hover::after { left: 100%; }
.btn-save-member:hover { transform: translateY(-2px); box-shadow: 0 10px 28px -6px rgba(14,165,233,0.5), 0 1px 0 #0891b2; }
.btn-save-member:active { transform: translateY(1px); box-shadow: 0 2px 6px rgba(0,0,0,0.15); }
.btn-save-member:disabled { opacity: 0.6; cursor: not-allowed !important; transform: none !important; pointer-events: auto !important; }
